...
class QLearning(AbstractSolver):
    ...
    def train_episode(self):
        ...
        for _ in range(self.options.steps):
            ...
            target = reward + self.options.gamma * np.max(self.Q[next_state]) 
    ...
    def epsilon_greedy_action(self, state):
        ...
        if np.random.rand() < epsilon:
            current_action = np.random.choice(nA)  # Uniformly choose between move and stay
        else:
            current_action = np.argmax(self.Q[state])
